...
const JibDetails = ({ params: id }) => {
    ...
    const getJobDetails = async () => {
        try {
            const response = await axiosInstance.get(
                `/job-post/job-detail/${id.id}`
            );
            console.log(response);
            if (response?.status === 200) {
                setSelectedJobDetails(response?.data?.response);
            }
        } catch (error) {
            console.error(error);
    ...
    const handelAppliedJob = async (values, { setSubmitting, resetForm }) => {
        try {
            ...
            const appliedResponse = await axiosInstance.post(
                '/job-post/applied-job',
                formData
            );
    ...
    return (
        <>
            ...
            <SocialShareModal
                setShareShowModal={setShowShareModal}
